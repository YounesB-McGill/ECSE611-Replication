...
def detectSmells(file, outputFile):
    detectMultifacetedAbs(file, outputFile)
...
def detectMultifacetedAbs(file, outputFile):
    detectMultifacetedAbsForm1(file, outputFile)
    detectMultifacetedAbsForm2(file, outputFile)

def detectUnnecessaryAbs(file, outputFile):
...
def detectImperativeAbs(file, outputFile):
    #for root, dirs, files in os.walk(folder):
     #   for file in files:
    #if file.endswith(".pp") and not os.path.islink(os.path.join(root, file)):
    if file.endswith(".pp"):
...
def detectMissingAbs(file, outputFile):
    #for root, dirs, files in os.walk(folder):
     #   for file in files:
    #if file.endswith(".pp") and not os.path.islink(os.path.join(root, file)):
    if file.endswith(".pp"):
...
def detectMultifacetedAbsForm1(file, outputFile):
    #for root, dirs, files in os.walk(folder):
    #    for file in files:
    if file.endswith(".pp"):
        fileObj = SourceModel.SM_File.SM_File(file)
...
def detectMultifacetedAbsForm2(file, outputFile):
    # for root, dirs, files in os.walk(folder):
    #     for file in files:
            #if file.endswith(".pp") and not os.path.islink(os.path.join(root, file)):
    if file.endswith(".pp"):
